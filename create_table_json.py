...
def agregar_create_table( json_file, path:str ):
    """Agrega el CREATE TABLE de cada tabla al archivo JSON y
       lo guarda en la ubicación indicada.
    ...
    print("AÑADIR CREATE TABLE HIVE")
    ...
    print("AÑADIR CREATE TABLE IMPALA")
    for table in tqdm(json_file.keys()):
    ...
    try:
        ...
        #Agregar loggs: ¿Se guardó el archivo?
        print(f'El archivo se guardó correctamente en {path}.json')
    except Exception as e:
        print('No ha sido posible guardar el archivo debido al siguiente error:')
        print(e)
        #Agregar loggs